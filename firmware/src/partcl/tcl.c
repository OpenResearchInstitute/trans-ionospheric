...
/**
 * Establish the global environment for variables
 */
...
/**
 * Prepends a new variable to the global environment as a linked list
 */
...
/**
 * Variable handler - deals with creating, changing, and retrieving variable values
...
int tcl_eval(struct tcl *tcl, const char *s, size_t len) {
  ...
  tcl_each(s, len, 1) {
    /*DBGPRINT("tcl_next %d %.*s\n", p.token, (int)(p.to - p.from), p.from);*/
    switch (p.token) {
    case TERROR:
      DBGPRINT("eval: FERROR, lexer error\n");
//      char message[256];
//      snprintf(message, 255, "Lexer Error: \"%s\"", s);
//      mbp_ui_error(message);
      return tcl_result(tcl, FERROR, tcl_alloc("", 0));
    case TWORD:
      DBGPRINT("token %.*s, length=%d, cur=%p (3.1.1)\n", (int)(p.to - p.from),
