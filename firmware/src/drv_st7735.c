...
#define ST7735_SCK_PIN		11
#define ST7735_MOSI_PIN		12
#define ST7735_PIN_CS		14
#define ST7735_PIN_DC		8
#define ST7735_PIN_LED		15
#define ST7735_PIN_RESET	13
...
static void __spi_event_handler(nrf_drv_spi_evt_t const * p_event, void *p_context) {
	...
	if (m_large_tx) {
		...
		nrf_drv_spi_transfer(&spi0, p_large_tx_data, count, &rx, 0);
...
static void inline __writeCommand(uint8_t command) {
	uint8_t rx;
	nrf_spim_tx_buffer_set(NRF_SPIM0, &command, 1);
	nrf_spim_rx_buffer_set(NRF_SPIM0, &rx, 0);
...
static void inline __writeData(uint8_t data) {
	uint8_t rx;
	nrf_spim_tx_buffer_set(NRF_SPIM0, &data, 1);
	nrf_spim_rx_buffer_set(NRF_SPIM0, &rx, 0);
...
void st7735_init() {
	...
	nrf_drv_spi_config_t spi_config0;
	spi_config0.sck_pin = ST7735_SCK_PIN;
	spi_config0.mosi_pin = ST7735_MOSI_PIN;
	spi_config0.miso_pin = NRF_DRV_SPI_PIN_NOT_USED;
	spi_config0.ss_pin = NRF_DRV_SPI_PIN_NOT_USED;
	spi_config0.irq_priority = SPI_DEFAULT_CONFIG_IRQ_PRIORITY;
	spi_config0.orc = 0x00;
	spi_config0.frequency = NRF_DRV_SPI_FREQ_8M;
...
inline void st7735_push_colors(uint8_t *p_colors, int32_t size) {
	...
	while (size > 0) {
		...
		m_busy = true;
		nrf_drv_spi_transfer(&spi0, p_colors, count, &rx, 0);
 ...
 */
void inline st7735_push_colors_fast(uint8_t *p_colors, int32_t size) {
	uint8_t rx = 0;
	uint8_t count = 0;
	...
	//Keep some state for the large transfer
//	if (count > 254) {
	m_large_tx = true;
...
void st7735_set_addr(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	...
	if (m_inverted) {
		__writeData16(y0 + ST7735_INVERTED_Y_OFFSET); // YSTART
		__writeData16(y1 + ST7735_INVERTED_Y_OFFSET); // YEND
 ...
 */
void st7735_start() {
	nrf_gpio_pin_set(ST7735_PIN_LED);
	...
	__writeData(ST7735_NORMAL_MADCTL);	//Set display orientation and RGB order
...
void st7735_inverted_set(bool inverted) {
	m_inverted = inverted;
	if (inverted) {
		__writeCommand(ST7735_MADCTL);
		__writeData(ST7735_INVERTED_MADCTL);
//		__writeCommand(ST7735_INVON);
//		__writeCommand(ST7735_INVOFF);
	} else {
		__writeCommand(ST7735_MADCTL);
		__writeData(ST7735_NORMAL_MADCTL);
//		__writeCommand(ST7735_INVON);
//		__writeCommand(ST7735_INVOFF);
	}
}
