...
{
    //name needs to be 9 or less in length for Trans-Ionospheric display
    char *adv_name = "MDT Radio";
    uint8_t name_len = (uint8_t)strlen(adv_name);

    //what we want to end up with:
    //uint8_t adv_data[31] = {0x02, 0x01, 0x06, 0x03, 0x19, 0x00, 0x40, 0x0a, 0x09, 0x4d, 0x44, 0x54, 0x20, 0x52, 0x61, 0x64, 0x69, 0x6f, 0x05, 0xff, 0x4a, 0x06, 0xde, 0xad};
    
    //uint8_t adv_data[31] = {0x02, 0x01, 0x06, 0x02, 0x19, 0x00, 0x40, 0x0, 0x09};
    
    //length, type, values
    //0x02 length, 0x01 type, 0x06 value
    //0x03 length, 0x19 type, 0x00, 0x40 value = 4000. Appearance field, abused to comply with Bender's scheme.
    //if there is a breaking change, then roll this number to 4000
    uint8_t adv_data[31] = {0x02, 0x01, 0x06, 0x03, 0x19, 0x00, 0x40};
    ...
    //commented out for TI
    adv_data[7] = name_len + 1;
    adv_data[8] = 0x09;
